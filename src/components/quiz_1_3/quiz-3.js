...
const results = [
    new Result("Вам подойдут персональные тренировки онлайн или заочное сопровождение в тренажерном зале. Так вы сможете заниматься из любой точки мира, не прерывая процесс." + `${gymCheat}`, "11"),
    new Result("Отдайте предпочтение онлайн персональным тренировкам: только вы и тренер в удобное время и в любом месте.", "10"),
    new Result("Идеальный вариант — очные занятия с тренером в тренажерном зале. И технику поставите, и над дисциплиной поработаете."+ `${gymCheat}`, "01"),
    new Result("Вы можете попробовать заниматься очно или выбрать заочное сопровождение в тренажерном зале. Легче всего прогрессировать за счет увеличения рабочего веса. Дома вы быстро упретесь в «потолок»."+ `${gymCheat}`, "00")
];
